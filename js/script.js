...
function validateForm() {
    ...
    if (DoB.value == "") {
        ...
        divDoBError.innerHTML = "Date of Birth is a required field.";
    ...
    if (ddCountry.value == "Yes") {
        ...
        if (txtCountry.value == "") {
            ...
            document.querySelector("#divCountryError").innerHTML = "Cannot be empty.";
    ...
    for (let i = 0; i < elements.length; i++) {
        for (let j = 0; j < invalidChars.length; j++) {
            if (elements[i].value.indexOf(invalidChars[j]) != -1){
                ...
                document.querySelector("#divInvalidCharError").classList.remove("invisible");
                formIsValid = false;
            }
        }
    }
